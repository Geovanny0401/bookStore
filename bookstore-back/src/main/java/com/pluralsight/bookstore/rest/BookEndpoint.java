...
@SwaggerDefinition(
        info=@Info(
           ...
           contact = @Contact(
                   ...
                   email="mgeovanny@uninorte.edu.co",
                   url="http://geovanny0401.blogspot.com.co"
           )
        ),
        host="localhost:8080",
...
public class BookEndpoint {
    ...
    @ApiOperation("Create a book given a JSon book representation")
    @ApiResponses({
    ...
    public Response createBook(Book book, @Context UriInfo uriInfo) {
        ...
        URI createURI = uriInfo.getRequestUriBuilder().path(book.getId().toString()).build();
        return Response.created(createURI).build();
    }

    @DELETE
    @Path("/{id : \\d+}")
    @ApiOperation("Delete a book given an id")
    @ApiResponses({
    ...
    @GET
    @Produces(APPLICATION_JSON)
    @ApiOperation(value = "Returns all the book",response = Book.class, responseContainer = "List")
    @ApiResponses({
    ...
    @Path("/count")
    @Produces(TEXT_PLAIN)
    @ApiOperation(value = "Returns the number of book",response = Long.class)
    @ApiResponses({
